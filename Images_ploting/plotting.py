...
def plot(img_path, record_id, speed, speed_zone, distance, direction, veh_type, veh_no, veh_ctg, voilation, operator_name, op_id, lat, long, col_b, col_g, col_r,loc):
    ...
    borderd_img = cv2.copyMakeBorder(img_read,30,60,0,0,borderType=cv2.BORDER_CONSTANT)
    ...
    cv2.putText(borderd_img, f"VEHICLE TYPE: {veh_type} ({veh_ctg}) , VEHICLE NO: {veh_no}", (10,740),font, fontScale, color, thickness)
    cv2.putText(borderd_img, f"VIOLATIONS: {voilation} ", (10,770),font, fontScale, color, thickness)
    cv2.putText(borderd_img, f"OPERATOR NAME: {operator_name}, OPERATOR DEPARTMENT: {op_id}, LOCATION: {loc}", (10,800),font, fontScale, color, thickness)
    cv2.imwrite("done_cust.png",borderd_img)
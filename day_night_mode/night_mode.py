...
def change_day_night_mode():
    try:
        ...
        if response.status_code == 200:
            return jsonify({"message": "Day/Night mode changed successfully."}), 200
        else:
            return jsonify({"error": f"Failed to change Day/Night mode. Status code: {response.status_code}"}), response.status_code
